...
def home_kb_inline() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="🏠 На главную", callback_data="home")
    return kb.as_markup()
...
def admin_confirm_buy(payment_id) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="Подтвердить оплату", callback_data=f"confirm_{payment_id}")
    kb.button(text="Отклонить оплату", callback_data=f"reject_{payment_id}")
    kb.adjust(1)
    return kb.as_markup()