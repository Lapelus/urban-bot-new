...
from bot.admin.kbs import admin_kb, admin_kb_back, product_management_kb, cancel_kb_inline, catalog_admin_kb, \
    admin_send_file_kb, admin_confirm_kb, dell_product_kb, admin_confirm_buy
from bot.admin.schemas import ProductModel, ProductIDModel, PurchaseIDModel
from bot.admin.utils import process_dell_text_msg
from bot.user.kbs import get_product_buy_kb, get_ticket_kb
...
async def process_about(call: CallbackQuery, session_with_commit: AsyncSession):
    _, payment_id = call.data.split('_')
    payment_data = await PurchaseDao.find_one_or_none_by_id(session=session_with_commit, data_id=int(payment_id))
    updated_payment_data = {
        'user_id': int(payment_data.user_id),
        'price': int(payment_data.price),
        'product_id': int(payment_data.product_id),
        'paid': True
    }

    await PurchaseDao.update(session=session_with_commit, filters=PurchaseIDModel(id=int(payment_id)), values=PaymentData(**updated_payment_data))
    await bot.send_message(chat_id=payment_data.user_id, text=f"✅ Ваша оплата подтверждена\nНомер оплаты: {payment_id}\n🥊 Нажмите, чтобы получить билет", reply_markup=get_ticket_kb(payment_id))



@admin_router.callback_query(F.data.startswith('reject_'))
async def reject_buy(call: CallbackQuery, session_with_commit: AsyncSession):
    _, payment_id = call.data.split('_')
    payment_data = await PurchaseDao.find_one_or_none_by_id(session=session_with_commit, data_id=int(payment_id))
    await bot.send_message(chat_id=payment_data.user_id, text="😔 Ваша оплата отклонена, попробуйте еще раз", reply_markup=get_product_buy_kb(payment_data.product_id, payment_data.user_id))